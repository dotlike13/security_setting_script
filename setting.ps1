...
Add-Content -Path test.inf -Value "PasswordHistorySize = 4"
...
Select-String "^MaximumPasswordAge" securitypolicy.txt | foreach{$_.line} >> ./$env:computername.txt 2>&1
...
for($index = 0; $index -lt $check_list.Length; $index++){
	$temp = Select-string $check_list[$index] ./$env:computername.txt | foreach{$_.line}
	if (-Not $temp){
